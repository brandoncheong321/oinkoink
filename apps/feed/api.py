...
def api_add_oink(request):
    ...
    # regex meaning: look for '@' symbol followed by word between 1-20 characters
...
def api_add_like(request):
    ...
    # checks if there is a like tied to the oink id: if it
    if not Like.objects.filter(oink_id = oink_id).filter(created_by = request.user).exists():
        ...
        create_notification(request, oink.created_by, 'mention')
